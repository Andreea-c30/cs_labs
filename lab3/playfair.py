...
def encrypt(message, matrix):
    ...
    while i < len(message) - 1:
        ...
        i += 2
    # last letter doesnt have a pair, add Q
    if len(message) % 2 != 0:
        message.append('Q')
    ...
    for i in range(0, len(message_list), 2):
        ...
        row1, col1 = letter_index(letter1, matrix)
        row2, col2 = letter_index(letter2, matrix)
        # when letters and on the same row
        if row1 == row2:
            message_list[i] = matrix[row1][(col1 + 1) % 5]
            message_list[i + 1] = matrix[row2][(col2 + 1) % 5]
...
def decrypt(ciphertext, matrix):
    ...
    for i in range(0, len(ciphertext_list), 2):
        ...
        # when letters and on the same row
        if row1 == row2:
            ciphertext_list[i] = matrix[row1][(col1 - 1) % 5]
            ciphertext_list[i + 1] = matrix[row2][(col2 - 1) % 5]
        # when letters and on the same column
        elif col1 == col2:
    ...
    if decrypted_message.endswith("Q"):
